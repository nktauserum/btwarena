...
typedef struct {
    uint16_t bitmap;
    size_t size, capacity; // capacity tells us how many buffers is in the pool, size - what size is each buffer 
    char* arena;
} Pool;

void pool_init(Pool* pool, size_t capacity, size_t size) {
    pool->capacity = capacity < 8 ? 8 : capacity;
    pool->size = size;
    
    pool->bitmap = ~0; // all fields are available
    pool->arena = (char*)malloc(capacity*size);         
    memset(pool->arena, 0, capacity * size);
}
...
void pool_put(Pool* pool, void* ptr) {
    ...
    if (offset < 0 || offset >= pool->capacity) return; // decline if ptr is possibly not within the arena
